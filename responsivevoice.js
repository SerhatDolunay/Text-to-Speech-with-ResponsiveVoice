...
(function (w, y) {
  ...
  function y(C, p) {
    ...
    "string" === typeof p
      ...
      : p instanceof Blob && p.type && q.setRequestHeader("Content-Type", p.type);
    try {
      q.send(p);
    } catch (z) {
      return !1;
  ...
  var ResponsiveVoice = function () {
      ...
      function y(b, c) {
        ...
        try {
          for (var h = 0; h < q.length; h++) {
            ...
            k
              ? (Array.isArray(k) || (k = [k]), c.collectionvoice && c.collectionvoice.name && -1 != k.indexOf(c.collectionvoice.name) && (g = !0))
              : (g = !0);
            var f = !1,
              l = e.systemvoices;
      ...
      a.numToWords = function (b) {
        ...
        return "number" === typeof b
          ...
          : "0" === b
          ? "zero"
          : k(l(3))(g)(Array.from(b)).map(F).map(x).filter(k(f)(d)).reverse().join(" ").trim();
      };
